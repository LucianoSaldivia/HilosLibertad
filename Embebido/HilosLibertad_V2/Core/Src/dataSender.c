	...
	switch( sender->state ){
		...
		case DS_WAITING_ANSWER:
			...
			if( flags->RxComplete ){
				...
				// Dejo de contar TimeOuts porque hubo respuesta
				sender->timeout_ctr = 0;
				sender->data_loss_timeouts_ctr = 0;
				// Si había alarma de timeout, la apago
				if( alarmIsOn(&(sender->alarma)) && getAlarmType(&(sender->alarma)) == TIMEOUT_ALARM_TYPE )
				{
				...
				switch( sender->rxBuffer ){
					...
					case ACK_CHAR:
						...
						// Apago la alarma, sin importar de que tipo sea
						stopAlarm( &(sender->alarma) );
						break;
